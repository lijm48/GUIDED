...
def main():
    ...
    for scale_factor in scale_factors:
        ...
        for dataset in datasets:
            dataset_root = paths.fgovd_benchmark
            command = f"CUDA_VISIBLE_DEVICES={args.gpu} python inference_on_benchmark.py --dataset {dataset_root}/{dataset}.json --out {dataset} --scale_factor {scale_factor} --model {args.model} --batch_size {args.batch_size} --cross_attention {args.cross_attention}"
            print(command)
            os.system(command)
