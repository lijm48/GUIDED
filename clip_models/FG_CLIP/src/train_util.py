...
def my_do_train(model,
    ...
    # data loading
    loader_lst = [DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle) for train_dataset in train_dataset_list]
    train_dataloader = CombinedDataLoader(loader_lst)
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
    if additional_val_dataset is not None:
        additional_batch_size = batch_size if isinstance(additional_val_dataset, PACCO2CLIPDataset) else batch_size // 4 # COCO must have lower bs
        additional_val_dataloader = DataLoader(additional_val_dataset, batch_size=additional_batch_size, shuffle=shuffle)
...
class CombinedDataLoader:
    """
    一个包装器类，用于组合多个 PyTorch DataLoader。
    它通过交替从输入 DataLoader 中获取批次来进行迭代。
    当一个 DataLoader 耗尽时，它会为其重新创建一个新的迭代器，
    并继续从下一个 DataLoader 获取数据，直到达到总批次数。

    Args:
        dataloaders (List[torch.utils.data.DataLoader]): 要组合的 DataLoader 实例列表。
    """
    def __init__(self, dataloaders: List[DataLoader]):
        if not dataloaders:
            raise ValueError("输入 dataloaders 列表不能为空")

        self.dataloaders = dataloaders
        self.num_dataloaders = len(dataloaders)

        # 计算长度：通常定义为最长的 dataloader 的长度乘以 dataloader 的数量，
        # 以确保最长的 dataloader 至少完整迭代一次。
        # 你也可以根据需要选择其他定义（例如，总和或最短长度）。
        try:
            # DataLoader 的 __len__ 返回的是批次的数量
            self._len = max(len(dl) for dl in self.dataloaders) * self.num_dataloaders
        except TypeError:
             # 如果有 DataLoader 不支持 len() (例如 IterableDataset 且未指定长度)
             # 你可能需要手动处理或设置一个默认行为。这里我们先抛出错误。
             raise TypeError("一个或多个提供的 DataLoader 不支持 len()")

        self._iterators = None
        self._loader_indices_cycle = None
        self._batches_yielded = 0

    def __len__(self) -> int:
        """
        返回此组合 DataLoader 在一个 epoch 中将产生的总批次数。
        """
        return self._len
    ...
    def __iter__(self) -> 'CombinedDataLoader':
        ...
        初始化并返回迭代器。
        """
        # 为每个 dataloader 创建新的迭代器
        self._iterators = [iter(dl) for dl in self.dataloaders]
        # 创建一个无限循环的索引，用于交替选择 dataloader (0, 1, ..., n-1, 0, 1, ...)
        self._loader_indices_cycle = itertools.cycle(range(self.num_dataloaders))
        # 重置已产生的批次数计数器
        self._batches_yielded = 0
        return self
    ...
    def __next__(self) -> Any:
        """
        返回下一个批次，交替从不同的 DataLoader 获取。
        """
        if self._iterators is None or self._loader_indices_cycle is None:
            raise RuntimeError("请先调用 iter() 来获取迭代器")

        # 检查是否已经产生了足够数量的批次
        if self._batches_yielded >= self._len:
            raise StopIteration

        # 获取下一个要从中采样的 dataloader 的索引
        current_loader_idx = next(self._loader_indices_cycle)
        current_iterator = self._iterators[current_loader_idx]

        try:
            # 尝试从当前选定的迭代器获取下一个批次
            batch = next(current_iterator)
        except StopIteration:
            # 如果当前的 DataLoader 迭代器耗尽了
            # 1. 为这个耗尽的 DataLoader 创建一个新的迭代器
            self._iterators[current_loader_idx] = iter(self.dataloaders[current_loader_idx])
            current_iterator = self._iterators[current_loader_idx]

            # 2. 再次尝试从新的迭代器获取批次
            #    （假设重新初始化的 dataloader 至少有一个批次）
            try:
                 batch = next(current_iterator)
            except StopIteration:
                 # 如果重新初始化后仍然立即耗尽（例如，原始 dataloader 长度为 0）
                 # 这表示存在问题或特殊情况。根据需要处理。
                 # 这里我们简单地再次抛出 StopIteration，但这通常不应发生，
                 # 因为 __len__ 的计算假设了长度 > 0。
                 # 或者可以跳过这个dataloader，但这会使长度计算复杂化。
                 # 最好的方式是确保输入的dataloader长度都 > 0。
                 raise RuntimeError(f"DataLoader at index {current_loader_idx} "
                                    f"yielded StopIteration immediately after reset.")
