...
from io_utils import read_json, write_json, save_object, load_object
...
if __name__ == "__main__":
    ...
    parser.add_argument('--visualize', action='store_true', help='If setted, the predictions are visualized')
    parser.add_argument('--visual_root', type=str, default='./visuals', help='Where results will be stored')
    args = parser.parse_args()
    
    pred_path = args.predictions
    dataset_path = args.ground_truth
    coco_path = "/data1/liangzhijia/datasets/coco"
    os.makedirs(args.visual_root, exist_ok=True)
    ...
    id_to_category_map = {str(cat['id']): cat['name'] for cat in test_set['categories']}
    ...
    for imm in test_set['images']:
        ...
        if args.visualize:
            ...
            _, idxs = torch.sort(pred['scores'], descending=True)
            idxs = idxs[:3]
            plt.close()
            fig, axis = plt.subplots(1,2, figsize=(10,5))
            axis[0].set_title("pred")
            axis[1].set_title("gt")
            axis[0].imshow(Image.open(os.path.join(coco_path, imm['file_name'])))
            axis[1].imshow(Image.open(os.path.join(coco_path, imm['file_name'])))
            
            show_bboxes(colors=['r'], axes=axis[0], bboxes=pred['boxes'][idxs], labels=[id_to_category_map.get(str(int(x)), str(int(x))) + f"({scr:.2f})" for x, scr in zip(pred['labels'][idxs], pred['scores'][idxs])])
            show_bboxes(colors=['b'], axes=axis[1], bboxes=target['boxes'], labels=[id_to_category_map.get(str(int(x)), str(int(x))) for x in target['labels']], is_xyxy=False)
            plt.savefig(args.visual_root + '/' + imm['file_name'].split('/')[-1].split('.')[0] + '.png')
