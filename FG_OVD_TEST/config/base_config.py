...
class Custom_DatasetMapper(DetrDatasetMapper):
    def __call__(self, dataset_dict):
        ...
        """
        # vis.print_json_structure(dataset_dict["annotations"])
        output_dict = super().__call__(dataset_dict)
        # print(f"ann cnt: {len(dataset_dict['annotations'])}")
        
        output_dict["level_vocab"] = dataset_dict["annotations"][0]["level_vocab"]
        output_dict["neg_vocab"] = dataset_dict["annotations"][0]["neg_category_name"]
        output_dict["file_name"] = dataset_dict["file_name"]
        output_dict["gt_boxes"] = dataset_dict["annotations"][0]["bbox"]
        return output_dict
