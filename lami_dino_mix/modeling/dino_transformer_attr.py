...
"""
DINOTransformerAttr: DINO Transformer with AttributeAttention.
...
Key difference vs. dino_transformer.py:
  ...
  - Builds an AttributeAttention module that aggregates per-category
...
class AttributeAttention(nn.Module):
    ...
    def forward(self, vocab_embed, vocab_embed_mask=None):
        ...
        # BUG-FIX 1: When all vocab entries for a category are padding (all zeros),
        # vocab_embed[:,0:1] is also a zero vector, so delta remains all zeros without NaN.
        # If vocab_embed itself contains NaN (rare), use nan_to_num as a fallback.
        ...
        # BUG-FIX 2: When a row has zero valid slots, softmax(-inf,...,-inf) produces NaN.
        # Solution: ensure at least one valid position per row (force index 0 valid),
        # then use masked_fill, so softmax never encounters an all-inf row.
        ...
        # # BUG-FIX 3: After softmax, NaN may still remain (extreme case), do another nan_to_num
...
class DINOTransformerAttr(nn.Module):
    """DINO Transformer with AttributeAttention for content-query enrichment.
    ...
    def __init__(
        ...
        # ---- AttributeAttention ----
        self.attr_attention = AttributeAttention(
    ...
    def _enrich_content_query(
        ...
        """Add AttributeAttention residual to content_query_embeds.
        ...
        if fg_vocab_emb is not None:
            # FG-OVD training path: per-entry atomic phrase embeddings provided at runtime
            ...
            # Inference path (FG_OVD_FLAG='modify'):
            # FG_inf.py attaches inference-time vocab to
            # transformer._infer_vocab_emb / _infer_vocab_mask before calling forward.
            # Clear immediately after use to avoid reusing stale data in next inference.
            ...
            # LVIS training path: look up pre-loaded per-category vocab embeddings
        ...
        # Size guard: vocab row count must match content_query row count, otherwise skip
    ...
    def forward(
        ...
        **kwargs,
        ...
        # ---- 4. Enrich content queries with AttributeAttention ----
