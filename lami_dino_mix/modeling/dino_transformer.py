...
class DINOTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.content_query_attention = AttributeAttention(
            head_query_dim=256,
            Vocab_embed_dim=768,
            expert_cnt=6,
        )
        ...
        self.attn = False
        # category_fg_vocabs_emb = torch.load(category_fg_vocabs_emb_path)
        # max_vocab_cnt = 0
        # for idx, fg_emb in category_fg_vocabs_emb.items():
        #     max_vocab_cnt = max(max_vocab_cnt, fg_emb.size(0))

        # self.category_fg_vocabs_emb = []
        # self.category_fg_vocabs_emb_mask = torch.zeros(len(category_fg_vocabs_emb.keys()), max_vocab_cnt, dtype=torch.bool).cuda()
        # for idx, vocab_emb in category_visual_vocabs_emb.items():
        #     paddeed_vocab_emb = torch.cat([
        #             vocab_emb,
        #             torch.zeros(max_vocab_cnt - vocab_emb.size(0), vocab_emb.size(1), dtype=torch.float32, device=vocab_emb.device)
