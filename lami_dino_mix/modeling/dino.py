...
class DINO(nn.Module):
    ...
    def __init__(
        ...
        class_cnt_per_ann: int = 11,
        CG_class_cnt: int = 100,
        ...
        lvis_class_name_json = json.load(open(f"{dataset_path}/lvis/lvis_v1_all_classes.json"))
        ...
        self.use_multi_vocab = False
        self.use_soft_min = False
    ...
    def set_lvis_classifier_batch_inputs(self, batched_inputs):
        # Re-implemented cluster fed loss: directly modify content_query_embedding, etc.
        ...
        prob[-1] = 0
        prob[:self.num_classes] = freq_weight.float().clone()
    ...
    def adjust_embeddings_to_threshold(self, a, b, threshold=1):
        """
        Move vector b so that its cosine similarity with a is at least `threshold`.

        Args:
            a (torch.Tensor): Embedding of shape (N, D), assumed norm 1.
            b (torch.Tensor): Embedding of shape (N, D), assumed norm 1.
            threshold (float): Target minimum cosine similarity.

        Returns:
            torch.Tensor: Adjusted b of shape (N, D), norm 1.
        """
    ...
    def clip_process_batch_inputs(self, batched_inputs, fg_n_neg=None, debug=False):
        # In coarse-grained CG_emb: batch * 11 (1 positive + 10 negative) embs per batch,
        # all 11 embs in each batch share the same subject.
        # In fine-grained FG_flatten_instances_name_emb: batch * 11 fine-grained embs.
        if fg_n_neg==None:
            fg_n_neg = 10
