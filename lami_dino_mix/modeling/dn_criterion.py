...
class DINOCriterion(TwoStageCriterion):
    ...
    def forward(self, outputs, targets, dn_metas=None, sub_list=None):
        ...
        """
        losses = super(DINOCriterion, self).forward(outputs, targets, sub_list)
        ...
        dn_losses = self.compute_dn_loss(dn_metas, targets, aux_num, num_boxes, sub_list)
        # if sub_list is None:
        losses.update(dn_losses)

        return losses

    def compute_dn_loss(self, dn_metas, targets, aux_num, num_boxes, sub_list):
        """
        ...
        if dn_metas and "output_known_lbs_bboxes" in dn_metas:
            ...
            for loss in self.losses:
                ...
                l_dict.update(
                    self.get_loss(
                        loss, output_known_lbs_bboxes, targets, dn_idx, num_boxes * dn_num, sub_list, **kwargs
                    )
                )
