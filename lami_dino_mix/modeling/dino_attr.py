# coding=utf-8
"""
DINOAttr: DINO with AttributeAttention enabled.
...
overrides only the transformer call so that:
  ...
  - LVIS branch  : the transformer looks up lvis_vocab_emb[content_inds]
                   internally and enriches content queries via AttributeAttention.
  - FG-OVD branch: the per-batch atomic-phrase embeddings built in
                   ...
                   AttributeAttention can work on them too.
...
class DINOAttr(DINO):
    """DINO + AttributeAttention for content query enrichment.
    ...
    def forward(self, batched_inputs, fg_n_neg=None, use_multi=True):
        ...
        FG-OVD data so that AttributeAttention inside DINOTransformerAttr can
        ...
        # ----------------------------------------------------------------
        # Determine AttributeAttention vocab inputs
        ...
        # ----------------------------------------------------------------
        # Feed into DINOTransformerAttr (note the fg_vocab_emb / fg_vocab_mask kwargs)
        # ----------------------------------------------------------------
        (
            inter_states,
            init_reference,
