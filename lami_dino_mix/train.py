...
def get_model_parameter_count(model):
    """
    Compute total parameter count, trainable parameter count, and trainable ratio.

    Args:
        model (torch.nn.Module): PyTorch model.

    Returns:
        tuple: (total_params, trainable_params, trainable_ratio).
    """
...
def eval_FG_1attribute_map(preds_list, n_neg=5, fg_name=None):
    ...
    dataset_path = f"{dataset_path}/FG_OVD/benchmarks/{fg_name}_with_subject.json"
